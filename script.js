...
const sendRequestToAPI = (xmlhttp) => {
    ...
    // const url = 'https://github.com/alelign962/CS498-DataVisualization/blob/main/data.json'
    const method  = "GET";
...
const createBars  =  (dates, gdps,scales) =>{
    ...
    //    .attr("class", (d, i) => i%2 === 0 ? "bar": "bar2")
       ...
       .on("mouseover", (e,d) => {
        let bill = d.toString().replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
        d3.select("#tooltip")
          .style("opacity", 0.85)
          .style("left", e.pageX + 6 + 'px')
          .style("top", e.pageY + 'px')
          .html(`<p>Date: ${dates[gdps.indexOf(d)]}</p><p>GDP: $${bill} Billion</p>`)
...
//xmlhttp. onload() will be called after send() function by the XHR infrastructur
xmlhttp.send ()
};
driver();